<script lang="ts">
	...
	const asyncUpdateTimeMixIdx = () => {
		setTimeout(() => {
			$timemixHeadIdx = $timemixHeadIdxTemp;
		}, 850);
	};
	...
	$: headStyles = Array.from({ length: $modelMeta.timemix_head_num }, (_, index) => ({
		...
		zIndex: $modelMeta.timemix_head_num - index,
	...
	let currentHeadIdx = $timemixHeadIdxTemp;

	timemixHeadIdxTemp.subscribe((newIdx) => {
		if (newIdx === currentHeadIdx) return;

		// 12->1
		if (currentHeadIdx === $modelMeta.timemix_head_num - 1 && newIdx === 0) {
			animateForwardTransition();
			currentHeadIdx = newIdx;
			return;
		}
		// 1->12
		if (newIdx === $modelMeta.timemix_head_num - 1 && currentHeadIdx === 0) {
	...
	const animateForwardTransition = () => {
		...
		// 	opacity: 0.6
		// });
		// tl.set('.sankey-top', { zIndex: $modelMeta.timemix_head_num - 1 });
		...
		// show content
		// tl.set('.sankey-top', { zIndex: $modelMeta.timemix_head_num });
		// tl.set(headContent, {
		// 	opacity: 1
		// });
		tl.set(restHeads, {
	...
	const onClickNext = () => {
		$timemixHeadIdxTemp =
			$timemixHeadIdxTemp < $modelMeta.timemix_head_num - 1 ? $timemixHeadIdxTemp + 1 : 0;
		// window.dataLayer.push({
		// 	event: `pagination-timemix-head-next`,
		// 	page_num: $timemixHeadIdxTemp,
		// 	pagination_name: 'timemix-head'
		// });
	};

	const onClickPrev = () => {
		$timemixHeadIdxTemp =
			$timemixHeadIdxTemp > 0 ? $timemixHeadIdxTemp - 1 : $modelMeta.timemix_head_num - 1;
		// window.dataLayer.push({
		// 	event: `pagination-timemix-head-prev`,
		// 	page_num: $timemixHeadIdxTemp,
		// 	pagination_name: 'timemix-head'
		// });
	};
...
<div class="multi-head flex w-full" data-click="timemix-head">
	<div class="head-title absolute bottom-2 right-3 text-right text-gray-400">
		<!-- <span class="title-text"
			>Head {$timemixHeadIdxTemp + 1} of {$modelMeta.timemix_head_num}</span
		>
		<button
			on:click={onClickPrev}
			disabled={$isOnAnimation || disablePagination}
			data-click="timemix-head-prev-btn"><AngleLeftOutline size="sm"></AngleLeftOutline></button
		>
		<button
			on:click={onClickNext}
			disabled={$isOnAnimation || disablePagination}
			data-click="timemix-head-next-btn"><AngleRightOutline size="sm"></AngleRightOutline></button
		> -->
	</div>
