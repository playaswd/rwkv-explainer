<script lang="ts">
	...
	import TimeMix from '~/components/TimeMix.svelte';
	...
	import { adjustTemperature, fakeRunWithCachedData, runModel_rwkv4 } from '~/utils/data';
	import { fetchAndMergeChunks } from '~/utils/fetchChunks';
	import WeightPopovers from '~/components/WeightPopovers.svelte';
	import { fade } from 'svelte/transition';
	import { ex0, ex1, ex2, ex3, ex4 } from '~/constants/examples/rwkv';
	import BlockTransition from '~/components/BlockTransition.svelte';
	import RKV from '~/components/RKV.svelte';

	import { pyodideInit, textToTokens, tokensToText, createOrtSession } from '~/utils/rwkv/code';
	...
	onMount(async () => {
			await pyodideInit();
			const rwkv4Tokenizer = {
				encode: textToTokens,
				decode: tokensToText
			};
			active = true;

			const unsubscribe = subscribeInputs_rwkv4(rwkv4Tokenizer);

			if (!$isMobile) {
				await fetchModel_rwkv4();
			}
	...
	// Fetch model onnx
	const fetchModel_rwkv4 = async () => {
		// Configure ORT Web WASM env to avoid COOP/COEP/threading issues
		ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web/dist/';
		ort.env.wasm.numThreads = 1; // disable threads to avoid cross-origin isolation requirement
		ort.env.wasm.proxy = true; // consistent with createOrtSession

		const chunkUrls = Array($modelMeta.chunkNum)
			.fill(0)
			.map((d, i) => `${base}/rwkv/${$selectedModel}/rwkv-4-pile-169m-uint8.onnx.part${i}`);
		...
		// Parameters for createOrtSession
		const backend = 'wasm';
		const n_layer = $modelMeta.layer_num;
		const n_embd = $modelMeta.dimension;
		const isOrtFile = false; // assuming not .ort file

		// Create ONNX session using createOrtSession
		const { session } = await createOrtSession(blob, isOrtFile, backend, n_layer, n_embd);
	...
	const subscribeInputs_rwkv4 = (tokenizer) => {
		const unsubscribeInputText = inputText.subscribe((value) => {
			if ($isFetchingModel || !$modelSession) {
				...
				// console.log('fake run:', value.trim());
			...
			// console.log('inputText changed:', value.trim());
			runModel_rwkv4({
				tokenizer,
				input: value.trim(),
				temperature: $temperature,
...
>
	{#if !!$expandedBlock.id}
		...
		></div>
	{/if}
	<div class="sankey opacity-1" class:timemix={$expandedBlock.id === 'timemix'}>
		<Sankey />
	</div>
		...
		<div class="steps" class:expanded={!!$expandedBlock.id} bind:offsetHeight={vizHeight}>
			...
			<div class="blocks relative">
				<div class="block-steps main" class:initial={$blockIdx === 0}>
					<RKV className="step" />
					<TimeMix className="step" />
					<ChannelMix className="step" />
				</div>
				<div
...
<style lang="scss">
	...
	.sankey {
		...
		&.timemix {
			:global(.sankey-top) {
				z-index: $EXPANDED_TIMEMIX_INDEX !important;
				pointer-events: none;
			}
			:global(.sankey-top > g) {
				opacity: 0.3;
			}
			:global(.sankey-top > g.timemix) {
				opacity: 1;
			}
		}
	}
